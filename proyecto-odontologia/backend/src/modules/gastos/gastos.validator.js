...
/*
  ABM 05 — Gastos. Validación de FORMATO (no de negocio).
  ...
  Las reglas de negocio (tipo activo, tratamiento del consultorio, monto
  inmutable, gasto no anulado, etc.) viven en gastos.service.js.
*/

const LIMITES = {
  monto: { max: 99999999.99 },
  descripcion: { max: DESCRIPCION_MAX },
...
*/
function validarIdGasto(req, res, next) {
...
function validarFiltrosListado(req, res, next) {
  ...
  if (
    imputacion !== undefined &&
    imputacion !== "" &&
    !IMPUTACION_FILTRO.includes(imputacion)
  ) {
    errores.push(
      `El filtro imputacion debe ser uno de: ${IMPUTACION_FILTRO.join(", ")}.`
    );
  }
...
/*
  Valida el cuerpo del alta de gasto.
*/
function validarDatosAlta(req, res, next) {
  const { id_tipo_gasto, id_tratamiento, monto, fecha_gasto, descripcion } =
    req.body || {};

  const errores = [];

  if (!esEnteroPositivo(id_tipo_gasto)) {
  ...
  if (
    ...
    id_tratamiento !== null &&
    id_tratamiento !== "" &&
    !esEnteroPositivo(id_tratamiento)
  ) {
    errores.push("El tratamiento indicado no es válido.");
  }

  if (fecha_gasto !== undefined && fecha_gasto !== null && fecha_gasto !== "") {
    if (!esFechaValida(fecha_gasto)) {
      errores.push("La fecha del gasto no es válida (YYYY-MM-DD).");
    }
  }
...
/*
  ...
  nivel formato; el service aplica las reglas (monto inmutable, gasto no anulado).
  id_tratamiento admite null / "" para pasar el gasto a general.
*/
function validarDatosEdicion(req, res, next) {
  const { id_tipo_gasto, id_tratamiento, fecha_gasto, descripcion } =
    req.body || {};

  const errores = [];

  if (
    id_tipo_gasto !== undefined &&
    id_tipo_gasto !== null &&
    id_tipo_gasto !== "" &&
    !esEnteroPositivo(id_tipo_gasto)
  ) {
    errores.push("El tipo de gasto no es válido.");
  }
