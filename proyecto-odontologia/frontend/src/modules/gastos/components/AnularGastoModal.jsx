...
/*
  Modal de anulación de un gasto. El motivo es obligatorio (mín. 5 caracteres),
  igual que la regla del backend. La anulación no se puede revertir.
*/

const MOTIVO_MIN = 5;

function AnularGastoModal({ abierto, gasto, cargando = false, onConfirmar, onCancelar }) {
      ...
      <div className="modal-confirmacion tratamientos-estado-modal">
        <div className="modal-confirmacion__contenido">
          ...
          <p>
            ...
            {gasto?.monto != null
              ...
              : ""}
            . Deja de contar para el total del período y para los reportes, y no
            se puede revertir.
          </p>
        </div>
        ...
        <div className="pacientes-form__cuerpo">
          <div className="roles-page__campo">
            <label htmlFor="gasto-motivo">Motivo de anulación *</label>
            <textarea
              id="gasto-motivo"
              rows={3}
              maxLength={255}
              value={motivo}
              disabled={cargando}
              placeholder="Ej.: Gasto cargado por error en el tipo equivocado"
              onChange={(e) => {
        ...
        <div className="modal-confirmacion__acciones">
          ...
          <button
            ...
            disabled={cargando}
          >
            {cargando ? "Anulando..." : "Anular gasto"}
          </button>
        </div>
      </div>
    </div>
...
export default AnularGastoModal;
